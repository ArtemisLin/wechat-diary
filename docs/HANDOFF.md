...
**🗄️ 2026-08-16 谷雨拍板: 本项目归档, 后续只做 020 插件版。** 理由: 插件版覆盖全部场景,
"不装 Obsidian 的用户"是假想用户 (68 个下载全来自商城), 双版本维护每改一处两遍不值得。
仓库标 archived, 不再打包发 Release, 不再修下面列的未回灌项。只读 MCP server 若要复用,
从这里拎出来独立即可 (它只读 md 文件夹, 不依赖本程序)。以下内容仅作历史记录。
