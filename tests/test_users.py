...
def test_load_valid_user(monkeypatch, tmp_path):
    ...
    _, users = _reload(monkeypatch, {"USER_ID": "u-abc", "DIARY_DIR": str(vault)})
    u = users.load("u-abc")
    assert u.user_id == "u-abc"
    assert u.diary_dir == vault
    assert vault.exists(), "diary_dir should be auto-created"


def test_load_unknown_user_raises(monkeypatch, tmp_path):
    _, users = _reload(monkeypatch, {"USER_ID": "u-abc", "DIARY_DIR": str(tmp_path)})
...
def test_load_missing_vault_raises(monkeypatch):
    _, users = _reload(monkeypatch, {"USER_ID": "u-abc", "DIARY_DIR": ""})
...
def test_timezone_defaults_to_shanghai(monkeypatch, tmp_path):
    ...
    config, _ = _reload(monkeypatch, {"USER_ID": "u-abc", "DIARY_DIR": str(tmp_path)})
...
def test_today_str_uses_timezone_not_system(monkeypatch, tmp_path):
    ...
    config, _ = _reload(monkeypatch, {"USER_ID": "u-abc", "DIARY_DIR": str(tmp_path), "TIMEZONE": "Asia/Shanghai"})
